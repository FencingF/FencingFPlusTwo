...
public class PopLag extends Module {
    ...
    public void onPop(TotemPopEvent event) {
        if (!getFencing().friendManager.isFriend(event.getPlayer().entityUniqueID) || !event.getPlayer().getName().equals(mc.player.getName()) || !fullNullCheck() || !send.getValue() || mc.player.getDistance(event.getPlayer().posX, mc.player.posY, event.getPlayer().posZ) > radius.getValue() || !timer.hasReached(delay.getValue())) return;
