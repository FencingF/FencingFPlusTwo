...
public class PopLag extends Module {
    ...
    public void onPacket(PacketEvent.Receive event) {
        if (event.getPacket() instanceof SPacketChat && cancel.getValue()) {
            ...
            if (text.contains("\u0B01") || text.contains("\u0201") || text.contains("\u2701") || text.equals(LAGMSG)) {
    ...
    public void onPop(TotemPopEvent event) {
        if (!getFencing().friendManager.isFriendByName(event.getPlayer().getName()) && !event.getPlayer().equals(mc.player) && send.getValue() && mc.player.getDistanceSq(event.getPlayer().getPosition()) <= radius.getValue() && timer.hasReached(delay.getValue())) {
            mc.player.connection.sendPacket(new CPacketChatMessage("/msg " + event.getPlayer().getName() + " " + LAGMSG));
            timer.reset();
            if (info.getValue()) ClientMessage.sendMessage(ChatFormatting.AQUA + "Attempted to lag " + event.getPlayer().getName() + ".");
        }
    }
}
